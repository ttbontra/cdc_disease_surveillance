...
# Build chart tables (Streamlit line_chart can’t do bands directly; we show three lines)
hist_df = pd.DataFrame({"date": hist_idx, "q10": q10_hist, "q50": q50_hist, "q90": q90_hist}).set_index("date")
fut_df = pd.DataFrame({"date": fut_idx, "q10": q10_fut, "q50": q50_fut, "q90": q90_fut}).set_index("date")
all_df = pd.concat([hist_df, fut_df], axis=0)

st.metric("Estimated R0 (rough)", round(float(out["rt0_est"]), 2))
st.line_chart(all_df[["q10", "q50", "q90"]])
